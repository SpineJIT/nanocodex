...
use crate::{
    ...
    tui::{PaneId, SpineInputLane, WorkerCommand, WorkerEvent},
};
...
async fn terminal_open_next_and_close_switch_the_durable_current_node() -> eyre::Result<()> {
    ...
    let (open_started, open_started_receiver) = tokio::sync::oneshot::channel();
    let (next_started, next_started_receiver) = tokio::sync::oneshot::channel();
    let (close_started, close_started_receiver) = tokio::sync::oneshot::channel();
    let open_release = Arc::new(Notify::new());
    let next_release = Arc::new(Notify::new());
    let close_release = Arc::new(Notify::new());
    let mut worker = SpineWorker::start_with_transition_gates(
        configured,
        Arc::clone(&runtime),
        intents,
        session_recipe,
        ...
        vec![
            (open_started, Arc::clone(&open_release)),
            (next_started, Arc::clone(&next_release)),
            (close_started, Arc::clone(&close_release)),
        ],
    );
    ...
    timeout(Duration::from_secs(5), open_started_receiver).await??;
    worker.commands().send(WorkerCommand::SpineInput {
        target: PaneId::Main,
        id: 2,
        prompt: "keep the child focused on the parser".into(),
        lane: SpineInputLane::Immediate,
    })?;
    open_release.notify_one();
    timeout(Duration::from_secs(5), next_started_receiver).await??;
    worker.commands().send(WorkerCommand::SpineInput {
        target: PaneId::Main,
        id: 3,
        prompt: "implement the next parser change directly".into(),
        lane: SpineInputLane::Immediate,
    })?;
    next_release.notify_one();
    timeout(Duration::from_secs(5), close_started_receiver).await??;
    worker.commands().send(WorkerCommand::SpineInput {
        target: PaneId::Main,
        id: 4,
        prompt: "return the completed fix to the parent".into(),
        lane: SpineInputLane::Immediate,
    })?;
...
async fn delivery_failures_after_commit_stop_the_spine_coordinator() -> eyre::Result<()> {
    for (fault, expected_claim) in [
        ...
        let directory = tempdir()?;
        let (worker, runtime) = scripted_spine_worker(directory.path(), Some(fault), None).await?;
        worker.commands().send(WorkerCommand::Prompt {
            target: PaneId::Main,
            prompt_id: 1,
...
#[tokio::test]
async fn factory_resume_restores_identity_and_delivers_unclaimed_continuation() -> eyre::Result<()>
{
    let directory = tempdir()?;
...
async fn factory_resume_aborts_and_delivers_a_persisted_prepared_transition() -> eyre::Result<()> {
    ...
    assert_eq!(
        ...
        Some("Recovered an uncommitted Spine transition; continuing from the last durable node.")
    );
...
#[tokio::test]
async fn factory_resume_accepts_a_claimed_manual_delivery_without_replaying_it() -> eyre::Result<()>
{
    let directory = tempdir()?;
    let (endpoint, server) = completed_responses_server(2).await?;
    let cli = spine_cli_with_websocket(&endpoint)?;
    ...
    let (root, _events) = Nanocodex::builder(openai)
        ...
        .tools_factory({
            ...
            move |agent| {
                ...
                drop(slot);
                let tools =
                    subagents::with_subagents(base_tools.clone(), agent, primary_agents.clone())?;
                with_spine_tools(tools, Arc::clone(&tool_sink))
            }
        })
        .child_tools_factory({
            let base_tools = base_tools.clone();
            move |agent| subagents::with_subagents(base_tools.clone(), agent, child_agents.clone())
        })
        .build()?;
...
#[tokio::test]
async fn prompt_submitted_during_a_transition_is_buffered_for_the_resulting_node()
-> eyre::Result<()> {
    let directory = tempdir()?;
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            match worker.events_mut().recv().await {
                Some(WorkerEvent::SpineInputBuffered {
                    target: PaneId::Main,
                    id: 2,
                    lane: SpineInputLane::Deferred,
                }) => {
                ...
                Some(_) => {}
                None => panic!("Spine worker stopped before buffering the queued prompt"),
            }
        }
    })
    .await?;
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            match worker.events_mut().recv().await {
                Some(WorkerEvent::SpineInputStarted {
                    target: PaneId::Main,
                    id: 2,
                    prompt,
                }) => {
                ...
                Some(_) => {}
                None => panic!("Spine worker stopped before routing the deferred prompt"),
            }
        }
    })
    .await?;
...
#[tokio::test]
async fn deferred_prompt_submitted_during_an_active_turn_is_buffered() -> eyre::Result<()> {
    let directory = tempdir()?;
...
#[tokio::test]
async fn enter_buffered_during_open_reaches_the_resulting_node() -> eyre::Result<()> {
    let directory = tempdir()?;
    let (started, started_receiver) = tokio::sync::oneshot::channel();
    let release = Arc::new(Notify::new());
    let handoff_seen = Arc::new(Notify::new());
    let mut worker = handoff_spine_worker(
        directory.path(),
        started,
        Arc::clone(&release),
        Arc::clone(&handoff_seen),
    )
    .await?;
    ...
    worker.commands().send(WorkerCommand::SpineInput {
        target: PaneId::Main,
        id: 2,
        prompt: "redirect the child scope".into(),
        lane: SpineInputLane::Immediate,
    })?;
    timeout(Duration::from_secs(5), async {
        loop {
    ...
    release.notify_one();
    timeout(Duration::from_secs(5), handoff_seen.notified()).await?;
    worker.shutdown().await?;
    Ok(())
}
...
async fn handoff_spine_worker(
    directory: &std::path::Path,
    started: tokio::sync::oneshot::Sender<()>,
    release: Arc<Notify>,
    handoff_seen: Arc<Notify>,
    ...
    let openai = OpenAi::builder("test-key")
        .service({
            let calls = Arc::clone(&calls);
            let handoff_seen = Arc::clone(&handoff_seen);
            move || HandoffSpineService {
                calls: Arc::clone(&calls),
                handoff_seen: Arc::clone(&handoff_seen),
            }
        })
        .build()?;
    let tools = Tools::builder().without_defaults().build()?;
    ...
    Ok(SpineWorker::start_with_transition_gate(
        ConfiguredAgent {
            handle: agent,
            events,
            realtime: None,
...
async fn cold_close_releases_the_child_bundle_before_restoring_the_parent() -> eyre::Result<()> {
    ...
    let open_delivery = runtime.commit(&open, child_session_id.clone(), None, "delivery-open")?;
        ...
        let output = match request.kind() {
            ...
            ResponsesAttemptKind::Generation => {
                match self.generation_calls.fetch_add(1, Ordering::Relaxed) {
                    ...
                    2 => {
                        assert_request_contains(&request, "the parser needs one-token lookahead");
    ...
    fn call(&mut self, request: ResponsesAttempt) -> Self::Future {
        ...
        let output = match (call, request.kind()) {
            ...
            (2, ResponsesAttemptKind::Generation) => {
                assert_request_contains(&request, "Scope:\\ninspect the parser");
                assert_request_contains(&request, "keep the child focused on the parser");
                code_generation(
                    "resp-child-next",
                    "call-child-exec",
            ...
            (3, ResponsesAttemptKind::Generation) => {
                ...
                assert_request_contains(&request, "implement the next parser change directly");
                code_generation(
                    "resp-sibling-close",
                    "call-sibling-exec",
            ...
            (4, ResponsesAttemptKind::Generation) => {
                ...
                assert_request_contains(&request, "return the completed fix to the parent");
...
impl Service<ResponsesAttempt> for HandoffSpineService {
    type Response = ResponsesServiceResponse;
    type Error = ResponseError;
    type Future = Ready<Result<ResponsesServiceResponse, ResponseError>>;
    ...
    fn call(&mut self, request: ResponsesAttempt) -> Self::Future {
        let output = match request.kind() {
            ...
            ResponsesAttemptKind::Generation => match self.calls.fetch_add(1, Ordering::Relaxed) {
                0 => code_generation(
                    "resp-root-open",
                    "call-root-open",
                    "await tools.spine__open({summary: 'inspect the child scope'});",
                ),
                1 => {
                    assert_request_contains(&request, "redirect the child scope");
                    self.handoff_seen.notify_one();
                    final_generation("resp-child-finished", "child handoff applied")
                }
                call => panic!("unexpected handoff generation {call}"),
            },
            _ => panic!("unexpected handoff Responses attempt"),
        };
        ready(Ok(ResponsesServiceResponse::new(output)))
    }
}
