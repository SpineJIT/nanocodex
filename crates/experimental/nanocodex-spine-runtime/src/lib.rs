...
use codex_spine_core::{
    NodeId, NodeKind, RawBoundary, RolloutEvent, SpineProjection, SpineReducer, ToolCallGroup,
    ToolOutcome, ToolUse,
};
...
impl SpineRuntime {
    ...
    pub fn open(&self, call_id: &str, summary: &str) -> Result<NodeId, SpineRuntimeError> {
        ...
        Ok(state.reducer.projection().cursor)
    }
    ...
    pub fn close(&self, call_id: &str, memory: &str) -> Result<SpineHandoff, SpineRuntimeError> {
        ...
        Ok(SpineHandoff {
            closed_node,
            live_node: state.reducer.projection().cursor,
        })
    }
