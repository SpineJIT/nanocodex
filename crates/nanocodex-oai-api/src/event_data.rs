...
///
/// Call [`crate::AgentEvent::data`] to obtain this view. The original raw
/// payload remains available on [`crate::AgentEvent`] for lossless JSONL
...
pub struct RunTerminal {
    ...
    /// Exact estimate and pricing provenance when configured.
...
/// diagnostic view, or serialize the containing [`crate::AgentEvent`]
/// unchanged.
#[derive(Clone, Debug)]
...
impl TransportEvent {
    pub(crate) fn new(kind: AgentEventKind, payload: Arc<RawValue>) -> Self {
        Self { kind, payload }
    }
