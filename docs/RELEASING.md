...
Nanocodex releases six crates.io packages in lockstep with the CLI binaries and
...
- The root `CHANGELOG.md` is generated by `git-cliff` from every commit, while
  each of the six published crates carries its own path-filtered
...
3. Generate the committed changelog after all intended feature commits are in
   ...
   `cargo-release`, refreshing the root changelog and all six crate-specific
...
to publish all six `nanocodex*` crates. Configure `nanocodex` on npm with the
