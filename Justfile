...
run-otel-detail: otel-up
    ...
    @OTEL_LEVEL="warn,nanocodex=info,nanocodex_oai_api=info,nanocodex_tools=info,nanocodex_mcp=info,nanocodex_stream_timing=trace" \
        cargo run --manifest-path bin/nanocodex/Cargo.toml -- \
        --otel-endpoint http://127.0.0.1:4318 \
        --otel-environment local-tui
...
release-check version:
    ...
    @for crate in nanocodex-oai-api nanocodex-core nanocodex-tools-macros nanocodex-observability nanocodex-service nanocodex-tools nanocodex-agent nanocodex; do \
